...
// by int64s, and the coordinates in those dimensions are non-negative
// integers.  Only a finite number of these coordinates are non-zero.
// Dimensions labeled by larger int64s are more significant.  
//
...
struct Time {
  ...
  bool never_;
  vector<pair<int64, int> > coordinates_;
  
  // times default to CREATION
  Time() { never_ = false; } 
  
  Time(const vector<pair<int64, int> > & coordinates) 
  :  never_(false), coordinates_(coordinates) {
    CHECK(VerifyOrderDimensions());
  }
  
  bool VerifyOrderDimensions() {
    for (uint i=0; i+1<coordinates_.size(); i++)
      if (coordinates_[i].first <= coordinates_[i+1].first) return false;
    return true;
  }

  // Manipulating times
  void Increment(int64 coordinate, int count);
...
inline Time operator + (const Time & t, int64 coordninate){
  Time ret = t;
